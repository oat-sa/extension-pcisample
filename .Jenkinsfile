    ...
    stages {
        stage('Resolve TAO dependencies') {
            environment {
                ...
                REPO_NAME='oat-sa/extension-pcisample'
            }
            steps {
                sh(
        ...
        stage('Tests') {
            parallel {
                stage('Backend Tests') {
                    when {
                        expression {
                            fileExists('build/pciSamples/test/unit')
                        }
                    }
                    agent {
                        docker {
                    ...
                    steps {
                        dir('build'){
                            sh(
                                ...
                                script: './vendor/bin/phpunit pciSamples/test/unit'
                            )
                        }
                    }
                }
                stage('Frontend Tests') {
                    when {
                        expression {
                            fileExists('build/pciSamples/views/build/grunt/test.js')
                        }
                    }
                    agent {
                        docker {
                    ...
                    steps {
                        dir('build/tao/views/build') {
                            ...
                            sh (
                                ...
                                script: 'npx grunt connect:test pcisamplestest'
                            )
                        }
                    }
                }
