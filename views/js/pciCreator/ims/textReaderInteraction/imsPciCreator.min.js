...
$editor.on("editorready",function(){resolve()})}))})}),Promise.all(editorsReady)}function toDataUrl(url,callback){return new Promise(function(resolve){var xhr=new XMLHttpRequest;xhr.onload=function(){var reader=new FileReader;reader.onloadend=function(){callback(reader.result),resolve()},reader.readAsDataURL(xhr.response)},xhr.open("GET",url),xhr.responseType="blob",xhr.send()})}function walk(node,action){var children=node.childNodes;action(node);for(var i=0;i<children.length;i++)walk(children[i],action)}function inlineResources(element,assetManager){var convertPromises=[];return walk(element,function(node){if(node.tagName&&"IMG"===node.tagName){
var src=node.getAttribute("src");if(src){var resolved=assetManager.resolve(src),convertPromise=toDataUrl(resolved,function(content){node.setAttribute("data-content",content)});convertPromises.push(convertPromise)}}}),convertPromises}function saveColumn(interaction,pageId,colIndex,text){var pageData=_.find(interaction.properties.pages,function(page){return parseInt(page.id,10)===parseInt(pageId,10)});pageData&&void 0!==pageData.content[colIndex]&&(pageData.content[colIndex]=text)}var stateQuestion=stateFactory.extend(Question,function(){function getTooltipInfos(tooltipId){
