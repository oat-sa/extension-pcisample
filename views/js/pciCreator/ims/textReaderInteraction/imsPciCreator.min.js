...
initEditors($container,interaction).then(function(){self.tooltips.init()}).catch(function(err){throw new Error("Error in editors initialisation "+err.message)})},function(){var widget=this.widget,$container=widget.$container,interaction=widget.element,creatorContext=widget.getCreatorContext();this.tooltips.destroy(),$container.off("."+interaction.typeIdentifier),containerEditor.destroy($container.find(".js-page-column")),creatorContext.trigger("registerBeforeSaveProcess",new Promise(function(resolve,reject){var assetManager=interaction.renderer.getAssetManager(),sources=[],contents={},promises=[];return interaction.properties.pages.forEach(function(page){
page.content.forEach(function(col){($.parseHTML(col,document.implementation.createHTMLDocument("virtual"))||[]).forEach(function(element){var container=document.createElement("div");container.appendChild(element),container.querySelectorAll("img").forEach(function(image){var src=image.getAttribute("src");src&&sources.push(src)})})})}),sources=sources.filter(function(source,i){return sources.indexOf(source)===i}),promises=sources.map(function(source){var previousContent=interaction.properties["content-"+source];return previousContent?(contents[source]=previousContent,Promise.resolve()):toDataUrl(assetManager.resolve(source)).then(function(content){
contents[source]=content})}),Promise.all(promises).then(function(){var content,property;for(property in interaction.properties)property.startsWith("content-")&&delete interaction.properties[property];for(content in contents)interaction.properties["content-"+content]=contents[content];resolve()}).catch(reject)}))});return stateQuestion.prototype.initForm=function(){function toggleNavigation(multiPages,navigation){multiPages="true"===multiPages||!0===multiPages,$(".js-navigation-select-panel").toggle(multiPages),$(".js-tab-position-panel").toggle(multiPages&&"buttons"!==navigation),$(".js-button-labels-panel").toggle(multiPages&&"tabs"!==navigation)}
var $positionSelect,_widget=this.widget,$form=_widget.$form,interaction=_widget.element;$form.html(formTpl(interaction.properties)),$(".js-page-height-select").val(interaction.properties.pageHeight),$(".js-tab-position").val(interaction.properties.tabsPosition),$(".js-navigation-select").val(interaction.properties.navigation),toggleNavigation(interaction.properties.multiPages,interaction.properties.navigation),"both"===interaction.properties.navigation&&($positionSelect=$(".js-tab-position"),$('select.js-tab-position option[value="bottom"]').attr("disabled","disabled"),$positionSelect.trigger("change")),formElement.initWidget($form),
