...
pagesSelector:".js-tab-content",tabButtonSelectior:".tr-tab-label",afterSelect:_.noop(),beforeSelect:_.noop(),afterCreate:_.noop(),beforeCreate:_.noop(),initialPageIndex:0};this.init=function(){function handleTabSelect($tab){const index=$tabs.index($tab);that.index(index)}_.isFunction(options.beforeCreate)&&options.beforeCreate.call(that),options=_.extend(defaultOptions,_.clone(options)),$tabs=$container.find(options.tabsSelector),$pages=$container.find(options.pagesSelector),currentTabIndex=options.initialPageIndex,this.index(currentTabIndex),$tabs.on("click",options.tabButtonSelectior,function(){handleTabSelect($(this).closest(options.tabsSelector))}),
$tabs.on("keypress",function(e){13!==e.which&&32!==e.which||handleTabSelect(this)}),_.isFunction(options.afterCreate)&&options.afterCreate.call(that)},this.index=function(index){return void 0===index?currentTabIndex:(index=parseInt(index,10),_.isFunction(options.beforeSelect)&&options.beforeSelect.call(that,index),currentTabIndex=index,$tabs.removeClass(options.activeButtonClass).addClass(options.buttonClass),$tabs.eq(index).addClass(options.activeButtonClass),$pages.hide(),$pages.eq(index).show(),_.isFunction(options.afterSelect)&&options.afterSelect.call(that,index),currentTabIndex)},this.countTabs=function(){return $tabs.length},this.init()}}),
function(window,document,undefined){!function(factory){"use strict";"function"==typeof define&&define.amd?define("taoQtiItem/portableLib/jquery.qtip",["taoQtiItem/portableLib/jquery_2_1_1"],factory):jQuery&&!jQuery.fn.qtip&&factory(jQuery)}(function($){"use strict";function QTip(target,options,id,attr){this.id=id,this.target=target,this.tooltip=NULL,this.elements={target:target},this._id=NAMESPACE+"-"+id,this.timers={img:{}},this.options=options,this.plugins={},this.cache={event:{},target:$(),disabled:FALSE,attr:attr,onTooltip:FALSE,lastClass:""},
this.rendered=this.destroyed=this.disabled=this.waiting=this.hiddenDuringWait=this.positioning=this.triggering=FALSE}function invalidOpt(a){return a===NULL||"object"!==$.type(a)}function invalidContent(c){return!($.isFunction(c)||c&&c.attr||c.length||"object"===$.type(c)&&(c.jquery||c.then))}function sanitizeOptions(opts){var content,text,ajax,once;return invalidOpt(opts)?FALSE:(invalidOpt(opts.metadata)&&(opts.metadata={type:opts.metadata}),"content"in opts&&(content=opts.content,invalidOpt(content)||content.jquery||content.done?(text=invalidContent(content)?FALSE:content,content=opts.content={text:text}):text=content.text,
