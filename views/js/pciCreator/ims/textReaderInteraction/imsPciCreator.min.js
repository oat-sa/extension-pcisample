...
$editor.on("editorready",function(){resolve()})}))})}),Promise.all(editorsReady)}function saveColumn(interaction,pageId,colIndex,text){var pageData=_.find(interaction.properties.pages,function(page){return parseInt(page.id,10)===parseInt(pageId,10)});pageData&&void 0!==pageData.content[colIndex]&&(pageData.content[colIndex]=text)}var stateQuestion=stateFactory.extend(Question,function(){function getTooltipInfos(tooltipId){var $tooltip=$container.find(".tooltip[data-identifier="+tooltipId+"]"),$tooltipColumn=$tooltip.closest(".js-page-column");return!(!$tooltip.length||!$tooltipColumn.length)&&{pageId:$tooltip.closest(".js-tab-content").data("page-id"),
colIndex:$tooltipColumn.data("page-col-index"),colHtml:htmlEditor.getData($tooltipColumn.find("[data-html-editable=true]"))}}var tooltipBuffer,self=this,$container=self.widget.$container,$form=self.widget.$form,interaction=self.widget.element,properties=interaction.properties,pageIds=_.pluck(properties.pages,"id"),maxPageId=Math.max.apply(null,pageIds);$container.on("click."+interaction.typeIdentifier,'[class*="js-add-page"]',function(){var num=properties.pages.length+1,$button=$(this),pageData={label:"Page "+num,content:["page "+num+" content"],id:++maxPageId},currentPage=0;containerEditor.destroy($container.find(".tr-passage")),
$button.hasClass("js-add-page-before")?properties.pages.unshift(pageData):$button.hasClass("js-add-page-after")&&(properties.pages.push(pageData),currentPage=properties.pages.length-1),interaction.widgetRenderer.renderAll(properties),interaction.widgetRenderer.tabsManager.index(currentPage)}),$container.on("click."+interaction.typeIdentifier,".js-remove-page",function(){var tabNum=$(this).data("page-num");containerEditor.destroy($container.find(".tr-passage")),properties.pages.splice(tabNum,1),interaction.widgetRenderer.renderAll(properties)}),$container.on("change."+interaction.typeIdentifier,".js-page-columns-select",function(){
...
initEditors($container,interaction).then(function(){self.tooltips.init()}).catch(function(err){throw new Error("Error in editors initialisation "+err.message)})},function(){var $container=this.widget.$container,interaction=this.widget.element;this.tooltips.destroy(),$container.off("."+interaction.typeIdentifier),containerEditor.destroy($container.find(".js-page-column"))});return stateQuestion.prototype.initForm=function(){function toggleNavigation(multiPages,navigation){multiPages="true"===multiPages||!0===multiPages,$(".js-navigation-select-panel").toggle(multiPages),$(".js-tab-position-panel").toggle(multiPages&&"buttons"!==navigation),
$(".js-button-labels-panel").toggle(multiPages&&"tabs"!==navigation)}var $positionSelect,_widget=this.widget,$form=_widget.$form,interaction=_widget.element;$form.html(formTpl(interaction.properties)),$(".js-page-height-select").val(interaction.properties.pageHeight),$(".js-tab-position").val(interaction.properties.tabsPosition),$(".js-navigation-select").val(interaction.properties.navigation),toggleNavigation(interaction.properties.multiPages,interaction.properties.navigation),"both"===interaction.properties.navigation&&($positionSelect=$(".js-tab-position"),$('select.js-tab-position option[value="bottom"]').attr("disabled","disabled"),
$positionSelect.trigger("change")),formElement.initWidget($form),formElement.setChangeCallbacks($form,interaction,{tabsPosition:function(i,value){i.properties.tabsPosition=value,i.widgetRenderer.renderAll(i.properties)},pageHeight:function(i,value){i.properties.pageHeight=value,i.widgetRenderer.renderPages(i.properties)},multiPages:function(i,value){toggleNavigation(value,i.properties.navigation),i.properties.multiPages=value,i.widgetRenderer.renderAll(i.properties)},navigation:function(i,value){toggleNavigation(i.properties.multiPages,value),"buttons"===value&&(i.properties.tabsPosition="top"),
...
var $content,contentHtml,$target=$(this),contentId=$target.attr("aria-describedBy");contentId&&($content=$container.find("#"+contentId),$content.length&&(contentHtml=$content.html(),$target.qtip({overwrite:!0,theme:"default",content:{text:contentHtml},position:{target:"mouse",my:"bottom center",at:"top center"}})))})}}}),define("taoQtiItem/portableLib/OAT/util/html",["taoQtiItem/portableLib/OAT/util/xml","taoQtiItem/portableLib/OAT/util/math","taoQtiItem/portableLib/OAT/util/tooltip"],function(xml,math,tooltip){"use strict";return{render:function($container){var markup=$container.html();markup=xml.removeNamespace(markup),$container.html(markup),
math.render($container),tooltip.render($container)}}}),define("textReaderInteraction/runtime/js/renderer",["taoQtiItem/portableLib/jquery_2_1_1","taoQtiItem/portableLib/lodash","taoQtiItem/portableLib/handlebars","textReaderInteraction/runtime/js/tabs","taoQtiItem/portableLib/OAT/util/html","taoQtiItem/portableLib/jquery.qtip"],function($,_,Handlebars,Tabs,htmlRenderer){"use strict";var fixMarkupMediaSources=function(html,renderer){return html=html||"",html.replace(/(<img[^>]*src=["'])([^"']+)(["'])/gi,function(substr,$1,$2,$3){return $1+(renderer.resolveUrl($2)||$2)+$3})};return function(options){function getHeight(element){
