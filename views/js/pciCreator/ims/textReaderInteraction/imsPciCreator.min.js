...
$editor.on("editorready",function(){resolve()})}))})}),Promise.all(editorsReady)}function toDataUrl(url){return new Promise(function(resolve){var xhr=new XMLHttpRequest;xhr.onload=function(){var reader=new FileReader;reader.onloadend=function(){resolve(reader.result)},reader.readAsDataURL(xhr.response)},xhr.open("GET",url),xhr.responseType="blob",xhr.send()})}function saveColumn(interaction,pageId,colIndex,text){var pageData=_.find(interaction.properties.pages,function(page){return parseInt(page.id,10)===parseInt(pageId,10)});pageData&&void 0!==pageData.content[colIndex]&&(pageData.content[colIndex]=text)}
var stateQuestion=stateFactory.extend(Question,function(){function getTooltipInfos(tooltipId){var $tooltip=$container.find(".tooltip[data-identifier="+tooltipId+"]"),$tooltipColumn=$tooltip.closest(".js-page-column");return!(!$tooltip.length||!$tooltipColumn.length)&&{pageId:$tooltip.closest(".js-tab-content").data("page-id"),colIndex:$tooltipColumn.data("page-col-index"),colHtml:htmlEditor.getData($tooltipColumn.find("[data-html-editable=true]"))}}
var tooltipBuffer,self=this,$container=self.widget.$container,$form=self.widget.$form,interaction=self.widget.element,properties=interaction.properties,pageIds=_.pluck(properties.pages,"id"),maxPageId=Math.max.apply(null,pageIds);$container.on("click."+interaction.typeIdentifier,'[class*="js-add-page"]',function(){var num=properties.pages.length+1,$button=$(this),pageData={label:"Page "+num,content:["page "+num+" content"],id:++maxPageId},currentPage=0;containerEditor.destroy($container.find(".tr-passage")),$button.hasClass("js-add-page-before")?properties.pages.unshift(pageData):$button.hasClass("js-add-page-after")&&(properties.pages.push(pageData),
currentPage=properties.pages.length-1),interaction.widgetRenderer.renderAll(properties),interaction.widgetRenderer.tabsManager.index(currentPage)}),$container.on("click."+interaction.typeIdentifier,".js-remove-page",function(){var tabNum=$(this).data("page-num");containerEditor.destroy($container.find(".tr-passage")),properties.pages.splice(tabNum,1),interaction.widgetRenderer.renderAll(properties)}),$container.on("change."+interaction.typeIdentifier,".js-page-columns-select",function(){
var colNum,numberOfColumns=parseInt($(this).val(),10),currentPageIndex=interaction.widgetRenderer.tabsManager.index(),currentCols=interaction.properties.pages[currentPageIndex].content,newCols=[],$page=$('[data-page-num="'+currentPageIndex+'"]');for(colNum=0;colNum<numberOfColumns;colNum++)newCols.push(currentCols[colNum]||"");newCols[numberOfColumns-1]+="<br>"+currentCols.slice(numberOfColumns).join("<br>"),$.each(newCols,function(key,val){var editor=$page.find('[data-page-col-index="'+key+'"] .container-editor').data("editor");editor&&editor.setData(val)}),interaction.properties.pages[currentPageIndex].content=newCols,
interaction.widgetRenderer.renderPages(interaction.properties),interaction.widgetRenderer.tabsManager.index(currentPageIndex)}),$container.on("selectpage."+interaction.typeIdentifier,function(event,currentPageIndex){var editor,pageIndex;$container.find(".js-page-column").each(function(){pageIndex=parseInt($(this).closest(".tr-page").data("page-num"),10),(editor=$(this).find(".container-editor").data("editor"))&&editor.setReadOnly(currentPageIndex!==pageIndex)})}),$container.on("beforerenderpages."+interaction.typeIdentifier,function(){containerEditor.destroy($container.find(".tr-passage"))}),$container.on("createpager."+interaction.typeIdentifier,function(){
initEditors($container,interaction)}),_.isArray(interaction.properties.tooltips)||(interaction.properties.tooltips=[]),this.tooltips=tooltipManager({$authoringContainer:$form.find(".tooltip_authoring"),$interactionContainer:$container,$editableFields:$container.find(".js-page-column"),tooltipsData:interaction.properties.tooltips}),this.tooltips.on("beforeDeleteTooltipMarkup",function(tooltipId){tooltipBuffer=getTooltipInfos(tooltipId)}),this.tooltips.on("afterDeleteTooltipMarkup",function(){tooltipBuffer&&(saveColumn(interaction,tooltipBuffer.pageId,tooltipBuffer.colIndex,tooltipBuffer.colHtml),tooltipBuffer=null)}),
initEditors($container,interaction).then(function(){self.tooltips.init()}).catch(function(err){throw new Error("Error in editors initialisation "+err.message)})},function(){var widget=this.widget,$container=widget.$container,interaction=widget.element,creatorContext=widget.getCreatorContext();this.tooltips.destroy(),$container.off("."+interaction.typeIdentifier),containerEditor.destroy($container.find(".js-page-column")),creatorContext.trigger("registerBeforeSaveProcess",new Promise(function(resolve){var assetManager=interaction.renderer.getAssetManager(),sources=[],contents={},promises=[];return interaction.properties.pages.forEach(function(page){
page.content.forEach(function(col){($.parseHTML(col,document.implementation.createHTMLDocument("virtual"))||[]).forEach(function(element){element.querySelectorAll&&element.querySelectorAll("img").forEach(function(image){var src=image.getAttribute("src");src&&sources.push(src)})})})}),sources=Array.from(new Set(sources)),promises=sources.map(function(source){var previousContent=(interaction.properties.contents||{})[source];return previousContent?(contents[source]=previousContent,Promise.resolve()):toDataUrl(assetManager.resolve(source)).then(function(content){contents[source]=content})}),Promise.all(promises).then(function(){
interaction.properties.contents=contents,resolve()})}))});return stateQuestion.prototype.initForm=function(){function toggleNavigation(multiPages,navigation){multiPages="true"===multiPages||!0===multiPages,$(".js-navigation-select-panel").toggle(multiPages),$(".js-tab-position-panel").toggle(multiPages&&"buttons"!==navigation),$(".js-button-labels-panel").toggle(multiPages&&"tabs"!==navigation)}var $positionSelect,_widget=this.widget,$form=_widget.$form,interaction=_widget.element;$form.html(formTpl(interaction.properties)),$(".js-page-height-select").val(interaction.properties.pageHeight),$(".js-tab-position").val(interaction.properties.tabsPosition),
