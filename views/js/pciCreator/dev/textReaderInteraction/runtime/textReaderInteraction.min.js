...
},this.trigger=function(event,data){events[event]&&_.forEach(events[event],function(e){e.callback.apply({type:event,ns:[]},data)})}}}),define("taoQtiItem/portableLib/OAT/util/event",["taoQtiItem/portableLib/OAT/util/EventMgr"],function(EventMgr){return{addEventMgr:function(instance){var eventMgr=new EventMgr;instance.on=function(event,callback){eventMgr.on(event,callback)},instance.off=function(event){eventMgr.off(event)},instance.trigger=function(event,data){eventMgr.trigger(event,data)}}}}),define("taoQtiItem/portableLib/handlebars",[],function(){return function(){var __module4__=function(){"use strict";function SafeString(string){this.string=string}
return SafeString.prototype.toString=function(){return""+this.string},SafeString}(),__module3__=function(__dependency1__){"use strict";function escapeChar(chr){return escape[chr]||"&amp;"}function extend(obj,value){for(var key in value)Object.prototype.hasOwnProperty.call(value,key)&&(obj[key]=value[key])}function escapeExpression(string){return string instanceof SafeString?string.toString():string||0===string?(string=""+string,possible.test(string)?string.replace(badChars,escapeChar):string):""}function isEmpty(value){return!value&&0!==value||!(!isArray(value)||0!==value.length)}var __exports__={},SafeString=__dependency1__,escape={"&":"&amp;","<":"&lt;",
">":"&gt;",'"':"&quot;","'":"&#x27;","`":"&#x60;"},badChars=/[&<>"'`]/g,possible=/[&<>"'`]/;__exports__.extend=extend;var toString=Object.prototype.toString;__exports__.toString=toString;var isFunction=function(value){return"function"==typeof value};isFunction(/x/)&&(isFunction=function(value){return"function"==typeof value&&"[object Function]"===toString.call(value)});var isFunction;__exports__.isFunction=isFunction;var isArray=Array.isArray||function(value){return!(!value||"object"!=typeof value)&&"[object Array]"===toString.call(value)};return __exports__.isArray=isArray,__exports__.escapeExpression=escapeExpression,__exports__.isEmpty=isEmpty,__exports__
}(__module4__),__module5__=function(){"use strict";function Exception(message,node){var line;node&&node.firstLine&&(line=node.firstLine,message+=" - "+line+":"+node.firstColumn);for(var tmp=Error.prototype.constructor.call(this,message),idx=0;idx<errorProps.length;idx++)this[errorProps[idx]]=tmp[errorProps[idx]];line&&(this.lineNumber=line,this.column=node.firstColumn)}var errorProps=["description","fileName","lineNumber","message","name","number","stack"];return Exception.prototype=new Error,Exception}(),__module2__=function(__dependency1__,__dependency2__){"use strict";function HandlebarsEnvironment(helpers,partials){this.helpers=helpers||{},
this.partials=partials||{},registerDefaultHelpers(this)}function registerDefaultHelpers(instance){instance.registerHelper("helperMissing",function(arg){if(2!==arguments.length)throw new Exception("Missing helper: '"+arg+"'")}),instance.registerHelper("blockHelperMissing",function(context,options){var inverse=options.inverse||function(){},fn=options.fn;return isFunction(context)&&(context=context.call(this)),!0===context?fn(this):!1===context||null==context?inverse(this):isArray(context)?context.length>0?instance.helpers.each(context,options):inverse(this):fn(context)}),instance.registerHelper("each",function(context,options){
var data,fn=options.fn,inverse=options.inverse,i=0,ret="";if(isFunction(context)&&(context=context.call(this)),options.data&&(data=createFrame(options.data)),context&&"object"==typeof context)if(isArray(context))for(var j=context.length;i<j;i++)data&&(data.index=i,data.first=0===i,data.last=i===context.length-1),ret+=fn(context[i],{data:data});else for(var key in context)context.hasOwnProperty(key)&&(data&&(data.key=key,data.index=i,data.first=0===i),ret+=fn(context[key],{data:data}),i++);return 0===i&&(ret=inverse(this)),ret}),instance.registerHelper("if",function(conditional,options){return isFunction(conditional)&&(conditional=conditional.call(this)),
...
var $content,contentHtml,$target=$(this),contentId=$target.attr("aria-describedBy"),qtipPositionTarget="event";if(contentId&&($content=$container.find("#"+contentId),$content.length)){parseInt($target.css("height"),10)/parseInt($target.css("fontSize"),10)>=2&&(qtipPositionTarget="mouse"),contentHtml=$content.html(),$target.qtip({overwrite:!0,theme:"default",content:{text:contentHtml},position:{target:qtipPositionTarget,my:"bottom center",at:"top center"}})}})}}}),define("taoQtiItem/portableLib/OAT/util/html",["taoQtiItem/portableLib/OAT/util/xml","taoQtiItem/portableLib/OAT/util/math","taoQtiItem/portableLib/OAT/util/tooltip"],function(xml,math,tooltip){
"use strict";return{render:function($container){var markup=$container.html();markup=xml.removeNamespace(markup),$container.html(markup),math.render($container),tooltip.render($container)}}}),define("textReaderInteraction/runtime/js/renderer",["taoQtiItem/portableLib/jquery_2_1_1","taoQtiItem/portableLib/lodash","taoQtiItem/portableLib/handlebars","textReaderInteraction/runtime/js/tabs","taoQtiItem/portableLib/OAT/util/html","taoQtiItem/portableLib/jquery.qtip"],function($,_,Handlebars,Tabs,htmlRenderer){"use strict";var fixMarkupMediaSources=function(html,renderer){return html=html||"",
html.replace(/(<img[^>]*src=["'])([^"']+)(["'])/gi,function(substr,$1,$2,$3){return $1+(renderer.resolveUrl($2)||$2)+$3})};return window.jQuery=$,function(options){function getHeight(element){var style=element.currentStyle||window.getComputedStyle(element),rect=element.getBoundingClientRect(),borderBox="border-box"===style.boxSizing;return rect.height+parseFloat(style.marginTop)+parseFloat(style.marginBottom)+(borderBox?0:parseFloat(style.paddingTop)+parseFloat(style.paddingBottom))+(borderBox?0:parseFloat(style.borderTopWidth)+parseFloat(style.borderBottomWidth))}function getExtraHeight(element){
...
;return"auto"!==pageHeight&&(pageHeight=parseInt(pageHeight,10),pageWrapperHeight=pageHeight+getWrapperHeight(multiPages)),{state:self.options.state,serial:self.options.serial,currentPage:currentPage+1,pagesNum:data.pages.length,multiPages:multiPages,showTabs:multiPages&&(data.pages.length>1||data.onePageNavigation)&&"buttons"!==data.navigation,showNavigation:multiPages&&(data.pages.length>1||data.onePageNavigation)&&"tabs"!==data.navigation,authoring:"question"===self.options.state,pageHeight:pageHeight,pageWrapperHeight:pageWrapperHeight,showRemovePageButton:data.pages.length>1&&"question"===self.options.state}},this.getTemplateOptions=function(){return{
helpers:{inc:function(value){return parseInt(value,10)+1}}}},this.init()}}),define("textReaderInteraction/runtime/js/textReaderInteraction",["taoQtiItem/portableLib/jquery_2_1_1","qtiCustomInteractionContext","taoQtiItem/portableLib/OAT/util/event","textReaderInteraction/runtime/js/renderer"],function($,qtiCustomInteractionContext,event,Renderer){"use strict";qtiCustomInteractionContext.register({id:-1,getTypeIdentifier:function(){return"textReaderInteraction"},initialize:function(id,dom,config){var pci=this._taoCustomInteraction;this.id=id,this.dom=dom,this.config=config||{},this.$container=$(dom),event.addEventMgr(this),
...
this.$container.off().empty()},setSerializedState:function(state){},getSerializedState:function(){return{}}})}),define(["textReaderInteraction/runtime/js/textReaderInteraction"],function(PCI){return PCI});
